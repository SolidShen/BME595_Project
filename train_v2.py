...
path_save_images = '/home/wang4001/dl_project/results_v2'
...
class SegmentationDataset(torch.utils.data.Dataset):
	...
	def __getitem__(self, index):
		...
		image = torch.tensor(self.loader(path_image,header=None).values,dtype = torch.float32)
...
for epoch in list(range(epochs)):
	...
	for batch_idx, (image,label) in enumerate(train_loader):
		...
		dice = dice_score(softmax(est_label),real_label.squeeze())
		# save results
		...
		# loss function is BCEloss: N * H * W * C
		est_label = est_label.permute(0, 2, 3, 1)
		real_label = real_label.permute(0 ,2, 3, 1)
		real_label_reshape = torch.ones(real_label.size()[0],real_label.size()[1],real_label.size()[2],num_classes,dtype = torch.float32)
		real_label_reshape[:,:,:,1] = torch.ones(real_label.size()[0],real_label.size()[1],real_label.size()[2],dtype = torch.float32) - real_label[:,:,:,0].type(torch.FloatTensor)
		real_label_reshape[:,:,:,0] = real_label[:,:,:,0].type(torch.FloatTensor)
		loss = loss_function(softmax(est_label),real_label_reshape)
	...
	parametersname = 'parameter_epoch'+str(epoch+1)
	torch.save(network_cuda,parametername)
